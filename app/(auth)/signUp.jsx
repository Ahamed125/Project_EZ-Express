...
const signUpSchema = z.object({
  username: z.string({ message: 'Username is required' }).min(4, 'Username should be at least 4 characters'),
  email: z.string({ message: 'Email is required' }).email('Invalid email'),
  password: z
    .string({ message: 'Password is required' })
    .min(8, 'Password should be at least 8 characters long'),
...
export default function SignUpScreen() {
  ...
  const onSignUp = async (data) => {
    ...
    try {
      ...
      if (isClerkAPIResponseError(err)) {
        if (err.errors.some(e => e.code === 'form_identifier_exists')) {
          Alert.alert('Error',  "That username is taken. Please try another.");
        } else {
          err.errors.forEach((error) => {
            setError('root', {
              message: error.message || 'An error occurred during sign up',
